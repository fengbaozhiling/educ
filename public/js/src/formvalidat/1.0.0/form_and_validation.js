...
define(function(require, exports, module) {
	...
	function validElement(ele){
		...
		var setErrMessage = function(ele,error_msg){
			...
			if(msgSpan !=undefined  && msgSpan != null){
				...
				//$(msgSpan).fadeOut("fast").fadeIn("fast");
			}else{
		...
		if(required && isNull){
			...
			if(lims != null){
				...
				for(i = 0;i<lims.length;i++){
					...
					if(lim[0] == "len"||lim[0] == "leng"){	// 长度检查，不管是什么类型，配置了长度就检查
						var t = lim[0] == "len"?1:2;//len按单字符计算，leng中文按双字符计算
						var lenDesc = lim[1];
						// alert(lim[1]);
	...
	function isMobileCN(s){
		...
		var patrn2 = /^(00852)?[-| ]?[6|9|5]{1}\d{7}$/;//香港手机
		var patrn3 = /^09\d{8}$/;//台湾手机
		var patrn4 = /^(00886)?[-| ]?0?9\d{8}$/;//台湾手机
		var patrn5 = /^(00853)?[-| ]?6\d{7}$/;//澳门手机
	...
	// 验证页面所有input标签
	function form_valid(){
		var eles = $("input,select,textarea"); // document.getElementsByTagName('input');
	...
	function ele_valid(id){
		...
		$("#"+id).blur(function(){
			...
			var value="";
			if(caseon=='submit'){
				aa = true; // 验证模式为只提交验证时 跳过验证
				$("#" + msgArea).removeClass('valid_info').removeClass('valid_error');// 去除错误显示
				$("#" + msgArea).html("<span>"+tips+"</span>");// 提示还原
			}
			else{
