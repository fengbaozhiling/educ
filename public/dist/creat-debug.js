define("educate/educate/1.0.0/creat-debug", [ "./user-debug", "./edstyle-debug", "./base-debug", "./lang-debug", "./jquery.lazyload-debug", "./login-debug", "./dialog/1.0.0/dialog-plus-debug", "./dialog/1.0.0/dialog-debug", "./dialog/1.0.0/popup-debug", "./dialog/1.0.0/dialog-config-debug", "./dialog/1.0.0/drag-debug", "./formvalidat/1.0.0/form_and_validation-debug", "./dot-debug", "./rating/jquery.rating-debug", "./swfobject-debug", "./jquery.bgiframe-debug" ], function(require, exports, module) {
    ...
    EDUCAT.uploadClient = function() {
        ...
        $(".UPLOAD_PHOTO_BOX_INIT").each(function() {
            ...
            if (swf.id == "") swf.id = swfId; else swfId = swf.id;
            ...
            swfobject.embedSWF(SITEURL + "public/swf/photo.swf", swfId, width, height, "11.5", SITEURL + "public/swf/expressInstall.swf", {
                siteUrl: SITEURL,
                maxSize: maxSize,
                imgCount: imgCount,
            ...
            $(this).removeClass("UPLOAD_PHOTO_BOX_INIT").addClass("UPLOAD_PHOTO_BOX").css({
                width: width,
                height: height
            });
        });
    ...
    $(".is_privacy").length && $(".is_privacy").change(function() {
        if (this.value == "0") $(".invite_code_box").hide(); else $(".invite_code_box").show();
    });
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (result.status == 1) {
                    ...
                    if (element) EDUCAT.ShowMiNiTooltip(element, "COURSE_DETAILS_TOOLTIP", result.msg, 0, 1); else EDUCAT.ShowMiNiTooltip($(".submit_button", form), "COURSE_DETAILS_TOOLTIP", result.msg, 0, 0);
                }
            },
            error: function() {
                EDUCAT.HideLoading(form);
    ...
    $("#sectionBox").sortable({
        ...
        stop: function(event, ui) {
            ...
            sectionSortTimeout = setTimeout(fun, 3e3);
        }
    });
    ...
    $(document).on("click", "#addSectionSubmit", function() {
        ...
        EDUCAT.ShowLoading(form);
        $.ajax({
            url: SITEURL + "index.php?m=coursemanage&a=createSection",
            //ajax/createSection.php
            type: "POST",
            data: form.serialize(),
            cache: false,
            dataType: "json",
            success: function(result) {
                ...
                if (result.status == 1) {
                    ...
                    if (typeof result.field != "undefined" && result.field == "name") EDUCAT.ShowMiNiTooltip($("#sectionName"), "SECTION_FORM_TOOLTIP", result.msg, 0, 1); else EDUCAT.ShowMiNiTooltip($("#addSectionSubmit"), "SECTION_FORM_TOOLTIP", result.msg, 0, 0);
                }
            },
            error: function() {
                EDUCAT.HideLoading(form);
        ...
        $.ajax({
            url: SITEURL + "index.php?m=coursemanage&a=updateSection",
            // ajax/updateSection.php
            type: "POST",
            data: form.serialize(),
            cache: false,
            dataType: "json",
            success: function(result) {
                ...
                if (result.status == 1) {
                    ...
                    if (typeof result.field != "undefined" && result.field == "name") EDUCAT.ShowMiNiTooltip($("#section_name"), "SECTION_FORM_TOOLTIP", result.msg, 0, 1); else EDUCAT.ShowMiNiTooltip($("#editSectionSubmit"), "SECTION_FORM_TOOLTIP", result.msg, 0, 0);
                }
            },
            error: function() {
                EDUCAT.HideLoading(box);
    ...
    }).on("click", "#editSectionClear", function() {
        EDUCAT.ResetSection();
        ...
        $.ajax({
            url: SITEURL + "index.php?m=coursemanage&a=deleteSection",
            // ajax/deleteSection.php
            type: "POST",
            data: query,
            cache: false,
            dataType: "json",
    ...
    EDUCAT.UpdateVodImageComplete = function(result) {
        if ($("#imgId").val() > 0) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.status == 1) {
                        $("#vodImgSwf").get(0).UploadLoadingTip(255, "更新图片成功", false);
                    } else {
                        $("#vodImgSwf").get(0).UploadLoadingTip(16711680, "更新图片失败", false);
                    }
                },
                error: function() {
                    $("#vodImgSwf").get(0).UploadLoadingTip(16711680, "更新图片失败", false);
            ...
            $("#vodImgSwf").get(0).UploadLoadingTip(255, "上传成功", false);
        }
    };
    $(document).on("click", ".add_vod_btn", function() {
    ...
    }).on("click", "#addVodClear", function() {
        EDUCAT.ResetVod();
    }).on("click", ".vod_edit_btn", function() {
        if (!EDUCAT.ResetVod()) return;
        var self = $(this);
    ...
    }).on("click", "#editVodSubmit", function() {
        var self = $(this);
        var form = self.parents("form");
    ...
    }).on("click", "#editVodClear", function() {
        EDUCAT.ResetVod();
    }).on("click", ".vod_remove_btn", function() {
        var box = $(this).parent();
    ...
    }).on("click", ".vod_video_btn", function() {
        if (!EDUCAT.ResetVod()) return;
        var self = $(this);
        ...
        EDUCAT.uploadClient();
    }).on("click", "#clearVodVideoBtn", function() {
        EDUCAT.ResetVod();
    });
    ...
    EDUCAT.VodVideoUploadSwf = function(rid) {
        ...
        swfobject.embedSWF("/public/swf/upload.swf", "uploadVodVideoSwf", "54", "34", "11.5", "/public/swf/expressInstall.swf", flashvars, params, attributes);
    };
    ...
    $("#addLive").length && $("#addLive").click(function() {
        ...
        $("#liveDurationSlider").slider({
            ...
            slide: function(event, ui) {
                ...
                var str;
                if (duration < 6) str = duration * 10 + "分钟"; else {
    ...
    EDUCAT.UpdateImageComplete = function(result) {
        if ($("#imgId").val() > 0) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.status == 1) {
                        $("#liveImgSwf").get(0).UploadLoadingTip(255, "更新图片成功", false);
                    } else {
                        $("#liveImgSwf").get(0).UploadLoadingTip(16711680, "更新图片失败", false);
                    }
                },
                error: function() {
                    $("#liveImgSwf").get(0).UploadLoadingTip(16711680, "更新图片失败", false);
            ...
            $("#imgId").val(result.id);
            $("#imgData").val(result.data);
            $("#liveImgSwf").get(0).UploadLoadingTip(255, "上传成功", false);
        }
    };
    ...
    var attachmentResourceId = 0;
    var attachmentHtml = "";
    $(document).on("click", ".lecture_attachment_btn", function() {
        if (!EDUCAT.AttachmentResetHandler()) return;
        var lid = $(this).attr("lid");
        if ($(".lecture_handler_" + lid).length > 0) {
            if (vodVideoUploadCount > 0) {
    ...
    $("a[data='course_publish']").click(function() {
        ...
        $.get(SITE_PATH + "index.php?m=coursemanage&a=publish", "id=" + courseId, function(result) {
            ...
            if (result.status == 0) {
                ...
                switch (result.field) {
                  ...
                  case "lecture":
                    EDUCAT.ShowConfirm("课程发布提示", result.msg, function() {
    ...
    $(".course_remove").click(function() {
        ...
        if (btn.hasClass("loading_btn")) return;
        EDUCAT.ShowMiniLoading(btn);
        EDUCAT.get(SITEURL + "index.php?m=coursemanage&a=remove", "id=" + courseId, function(result) {
            EDUCAT.HideMiniLoading(btn);
            if (result.status == 0) {
...
define("educate/educate/1.0.0/user-debug", [ "educate/educate/1.0.0/edstyle-debug", "educate/educate/1.0.0/base-debug", "educate/educate/1.0.0/lang-debug", "educate/educate/1.0.0/jquery.lazyload-debug", "educate/educate/1.0.0/login-debug", "educate/educate/1.0.0/dialog/1.0.0/dialog-plus-debug", "educate/educate/1.0.0/dialog/1.0.0/dialog-debug", "educate/educate/1.0.0/dialog/1.0.0/popup-debug", "educate/educate/1.0.0/dialog/1.0.0/dialog-config-debug", "educate/educate/1.0.0/dialog/1.0.0/drag-debug", "educate/educate/1.0.0/formvalidat/1.0.0/form_and_validation-debug", "educate/educate/1.0.0/dot-debug", "educate/educate/1.0.0/rating/jquery.rating-debug" ], function(require, exports, module) {
    var edstyle = require("educate/educate/1.0.0/edstyle-debug");
    var edstyle = require("educate/educate/1.0.0/login-debug");
    var dot = require("educate/educate/1.0.0/dot-debug");
    var dialog = require("educate/educate/1.0.0/dialog/1.0.0/dialog-plus-debug");
    var rating = require("educate/educate/1.0.0/rating/jquery.rating-debug");
    //星星评价
...
*/
define("educate/educate/1.0.0/swfobject-debug", [], function(require) {
...
define("educate/educate/1.0.0/jquery.bgiframe-debug", [], function(require) {
    return function(jquery) {
        (function(t) {
            t.fn.bgIframe = t.fn.bgiframe = function(e) {
                if (/msie 6.0/i.test(navigator.userAgent)) {
                    ...
                    }, r = '<iframe class="bgiframe"frameborder="0"tabindex="-1"src="' + e.src + '"' + 'style="display:block;position:absolute;z-index:-1;' + (e.opacity !== !1 ? "filter:Alpha(Opacity='0');" : "") + "top:" + ("auto" == e.top ? "expression(((parseInt(this.parentNode.currentStyle.borderTopWidth)||0)*-1)+'px')" : i(e.top)) + ";" + "left:" + ("auto" == e.left ? "expression(((parseInt(this.parentNode.currentStyle.borderLeftWidth)||0)*-1)+'px')" : i(e.left)) + ";" + "width:" + ("auto" == e.width ? "expression(this.parentNode.offsetWidth+'px')" : i(e.width)) + ";" + "height:" + ("auto" == e.height ? "expression(this.parentNode.offsetHeight+'px')" : i(e.height)) + ";" + '"/>';
                    return this.each(function() {
                        0 == t("> iframe.bgiframe", this).length && this.insertBefore(document.createElement(r), this.firstChild);
                    });
                }
                return this;
            };
